...
var CloseIO = module.exports = integration('CloseIO.io')
...
CloseIO.prototype.identify = function (identify, settings, callback) {
  ...
  this._getLead({ query : 'email_address:' +  email }, settings, function (err, lead) {
    ...
    if (!lead) {
      ...
      var contactId = lead.contacts.find(function(contact){
...
/**
 * Updates the Lead in CloseIO.io with the new identify
 *
 * @param  {String}            id        the CloseIO.io Lead id
...
/**
 * Updates the Contact in CloseIO.io with the new identify
 *
 * @param  {String}   id        the CloseIO.io Contact id
...
/**
 * Creates a Contact in CloseIO.io
 *
 * We first check and see if their is a Lead in CloseIO.io with a URL
 ...
 * @api public
 */
CloseIO.prototype._createNote = update('/activity/note/');
