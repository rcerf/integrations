    ...
    if (!lead) self._createContact(identify, settings, callback);
      ...
      lead.contacts.forEach(function(contact){
        if(contact.emails.reduce(function(prev, curr){
          ...
          identify.closeId = contact.id;
          console.log('+++++++++++++++++++++++++SET CONTACT ID+++++++++++++++++', identify, '\n');
        }
      })
...
function request(method, path){
    ...
    return function(payload, settings, fn){
      return this
        ...
        .end(this.handle(function(err, res){
          if (err) return fn(err);
          if(path === '/lead/') console.log('**********GET RES********\n', res.body);
  ...
  if(method === 'PUT'){
    return function(payload, settings, fn){
      if(/cont/.test(payload.closeId)) debugger;
      var closeId = payload.closeId + '/';
      payload.closeId = undefined;
