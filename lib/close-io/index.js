...
Close.prototype._createContact = function(identify, settings, fn){
  console.log('***IDENTIFY IN CREATECONTACT***: ', identify);
  var company = identify.custom.company
    , self = this;
  console.log('***COMPANY***: ', company);
  if (identify.lead_id) {
    identify.custom = undefined;
    console.log('***WE HAVE A LEAD ID****', identify);
    request('POST', '/contact/')(identify, settings, fn);
  }
  if (!!company) {
    this._getLead({query: 'company:' +'"'+ company + '"'}, settings, function (err, lead){
      console.log('***LEAD in CREATECONTACT under GETLEAD***', lead);
      if (err) return fn(err);
      if (lead) {
        identify.lead_id = lead.id;
        self._createContact(identify, settings, fn)
      }
      else self._createLead(transform(identify), settings, function (err, lead) {
        identify.lead_id = lead.id;
        if (err) return fn(err);
        self._createContact(identify, settings, fn);
      });
